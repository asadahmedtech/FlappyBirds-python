    ...
    while not gameExit:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                gameExit = True
                gameOver = False
            if (event.type == pygame.KEYUP and( event.key == pygame.K_UP or event.key == pygame.K_SPACE)) or event.type == pygame.MOUSEBUTTONDOWN:
                y = 40
                print "IN UP"
        ...
        if bird_x == x_moving-15 and bird_y1 >= length_y+gap:
            gameOver = True
        if ((x_moving-15 < bird_x < x_moving+100-15 and bird_y1 <= length_y-15) or (x_moving-15 < bird_x < x_moving+100-15 and bird_y1 >= length_y+gap-15)):
            gameOver = True
